...
export function obtenerMejorAlumno(alumnos: Alumno[]): Alumno | undefined {
  if (alumnos != null) {
    return alumnos.find(alumno => alumno.nota === Math.max(...alumnos.map(alumno => alumno.nota)));
...
export function calcularPromedioPorCiudad(alumnos: Alumno[], ciudad: string): number {
  let filtrados = obtenerAlumnosDeCiudad(alumnos, ciudad);
  if (filtrados !== null) return calcularPromedio(filtrados)
  else return 0;
}
